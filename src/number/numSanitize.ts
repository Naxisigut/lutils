...
function createCtx(val: string, option?: SanitizeOption){
  ...
  const ctx = {
    ...
    docking(index: number){ // 去除尾部，不包含index对应的值
...
function getChType(ctx: any, index: number){
...
function getMinusCommand(ctx: any){
...
function getDotCommand(ctx: any){
...
function getNumCommand(ctx: any){
